...
export default function Home() {
  ...
  const fetchCats = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/animal/categories');
  ...
  const fetchAnimalCategoryWise = async (catID) => {
    try {
      const response = await axios.get(`http://localhost:3001/api/animals/${catID}`);
      setAnimals(response.data?.animals || []);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  ...
  const addCat = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/api/animal/categories', {
      ...
      <main  style={{height: '500px', backgroundColor: 'gray'}}>
          ...
          {cats.length > 0 && (
            <ul>
              {cats?.map((category, index) => (
                <button style={{margin:5}} key={index} onClick={()=> setCatID(category?._id)}>{category.category_name}</button>
              ))}
            </ul>
          )}
        ...
        {animals.length > 0 && (
            <ul>
              {animals.map((animal, index) => (
                ...
                >
                <img src={`http://localhost:3001/${animal.animal_image}`} alt={animal.animal_name} style={{ width: '100px', height: '100px', marginRight: '10px' }}/>
                  {animal.animal_name}
                </span>
                </>
              ))}
        ...
        <div>
          ...
          <form onSubmit={addCat}>
            <input placeholder='Name' onChange={(e) => setNewCat(e.target.value)} />
        ...
        <div>
          ...
          <select id="catSelect">
            ...
            <input placeholder='Name' onChange={(e) => setNewCat(e.target.value)} />
            <input placeholder='Name' type='file' onChange={(e) => setNewCat(e.target.value)} />
            <button type="submit">Save</button>
          </form>
        </div>
